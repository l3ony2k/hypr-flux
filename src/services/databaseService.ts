...
export class DatabaseService {
  ...
  async saveImage(
    ...
    try {
      ...
      // Extract only relevant settings, avoiding storing full base64 data
      const { imageData, settings, ...otherMeta } = image;

      // Create a clean settings object that doesn't include binary data or empty values
      // but preserves only necessary model-specific parameters
      const cleanSettings = Object.entries(settings)
        .filter(([key, value]) => {
          ...
          // Skip common non-essential fields that shouldn't be stored
          if (key === "raw" && value === false) return false;
          return true;
        })
        .reduce((obj, [key, value]) => ({ ...obj, [key]: value }), {});

      // Replace binary data fields with placeholder values only
      ...
      // Create lean metadata object without unnecessary fields
      const metaToStore = {
        ...otherMeta,
      ...
      if (prepend) {
        metadata.unshift(metaToStore);
      } else {
        metadata.push(metaToStore);
      }
  ...
  async importImages(images: GeneratedImage[]): Promise<void> {
    ...
    try {
      ...
      const existingTimestamps = new Set(
        existingMetadata.map((m: any) => m.timestamp),
      );

      // Filter out duplicates while maintaining order
      const newImages = images.filter(
        (img) => !existingTimestamps.has(img.timestamp),
      );
    ...
    try {
      ...
      if (savedMetadata) {
        ...
        const updatedMetadata = metadata.filter(
          (item: Omit<GeneratedImage, "imageData">) =>
